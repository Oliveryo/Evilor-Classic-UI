...
local revision = tonumber(string.sub("$Revision: 2303 $", 12, -3))
if revision > Cartographer.revision then
	Cartographer.version = "r" .. revision
	Cartographer.revision = revision
	Cartographer.date = string.sub("$Date: 2009-02-19 07:46:53 +0000 (Thu, 19 Feb 2009) $", 8, 17)
end
				...
				for instance in Tourist:IterateZoneInstances(zone) do
					...
					if low == high then
						if (altGroupSize > 0) and (groupSize > 0) then
							table.insert(t, string.format("|cff%02x%02x%02x%s|r |cff%02x%02x%02x[%d]|r " .. L["%d-man"] .. " and " .. L["%d-man"], r1 * 255, g1 * 255, b1 * 255, name, r2 * 255, g2 * 255, b2 * 255, high, groupSize, altGroupSize))
						...
						if (altGroupSize > 0) and (groupSize > 0) then
							table.insert(t, string.format("|cff%02x%02x%02x%s|r |cff%02x%02x%02x[%d-%d]|r " .. L["%d-man"] .. " and " .. L["%d-man"], r1 * 255, g1 * 255, b1 * 255, name, r2 * 255, g2 * 255, b2 * 255, low, high, groupSize, altGroupSize))
						elseif groupSize > 0 then
