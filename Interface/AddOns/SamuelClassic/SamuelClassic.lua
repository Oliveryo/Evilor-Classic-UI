...
local _default_width = 300;
local _default_height = 18;
...
local _resetSwingTimerOnSpellDamage = function(spell)
	-- If it's in the list, it should reset the swingtimer.
	if _swing_reset_actions[spell] then
		_resetSwingTimer();
	end
end
...
--------
-- #TODO: This looks an awfull lot like a class... again.
...
local _eventCoordinator = function(self, event, arg1, ...)
	-- given:
...
local _onCombatEvent = function (self, event, arg1, ...)
	if (event == "COMBAT_LOG_EVENT_UNFILTERED") then
		...
		if (sourceGUID == UnitGUID("player")) then
			if (string.find(subEvent, "SWING.*") ~= nil) then
				_resetSwingTimer()
			elseif (string.find(subEvent, "SPELL.*") ~= nil) then
				spell = select(13, CombatLogGetCurrentEventInfo())
				_resetSwingTimerOnSpellDamage(spell)
			end
		elseif (destGUID == UnitGUID("player") and sourceGUID == UnitGUID("target")) then
			if (UnitIsPlayer("target")) then
				if (string.find(subEvent, "SPELL.*") ~= nil) then
					spell = select(13, CombatLogGetCurrentEventInfo())
					_resetSwingTimerOnSpellDamage(spell)
				end
			end
...
local _populateRequiredEvents = function()
	_addEvent("COMBAT_LOG_EVENT_UNFILTERED", _onCombatEvent);
